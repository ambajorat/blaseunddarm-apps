    ...
    private func exportPDF() {
        ...
        let data = renderer.pdfData { ctx in
            ...
            if !symptomEntries.isEmpty {
                let red = UIColor(red: 0.78, green: 0.29, blue: 0.24, alpha: 1)
                let warnTitle: [NSAttributedString.Key: Any] = [.font: UIFont.boldSystemFont(ofSize: 14), .foregroundColor: red]
                "Auffälligkeiten im Zeitraum".draw(at: CGPoint(x: m, y: y), withAttributes: warnTitle)
                y += 20
                let warnLine: [NSAttributedString.Key: Any] = [.font: UIFont.systemFont(ofSize: 10), .foregroundColor: UIColor.darkGray]
                for e in symptomEntries.prefix(8) {
                    let syms = (e.symptoms ?? []).map(\.rawValue).joined(separator: ", ")
                    "\(tf.string(from: e.timestamp)) — \(syms)".draw(at: CGPoint(x: m + 4, y: y), withAttributes: warnLine)
                    y += 14
                }
                if symptomEntries.count > 8 {
                    "… und \(symptomEntries.count - 8) weitere".draw(at: CGPoint(x: m + 4, y: y), withAttributes: warnLine)
                    y += 14
                }
                y += 10
            }
